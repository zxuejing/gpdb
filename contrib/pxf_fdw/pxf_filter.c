 ...
 */
				...
				pxf_free_expression_items_list(List *expressionItems)
	{
		...
		while (list_length(expressionItems) > 0)
		{
			...
			/*
			 * to avoid freeing already freed items - delete all occurrences
 ...
 */
	static List *pxf_make_expression_items_list(List *quals, Node *parent)
	{
		...
		{
			/*
			 * Planner (but not ORCA) will omit AND operators at the root
			 * level, so if we find more than 1 qualifier,
			 */
			/*
 ...
 *
 */
	static char *pxf_serialize_filter_list(List *expressionItems)
	{
		StringInfo	resbuf;
		ListCell   *lc = NULL;
				...
				case T_Var:
					{
						...
						if (var_to_pxffilter(var, filter))
						{
							...
							{
								/*
								 * var_to_pxffilter() should have never let
								 * this happen
								 */
								elog(ERROR,
						...
						{
							/*
							 * if at least one expression item is not
							 * supported, whole filter doesn't make sense
							 */
				...
				case T_ScalarArrayOpExpr:
					{
						...
						if (scalar_array_op_expr_to_pxffilter(expr, filter))
						{
							...
							{
								/*
								 * scalararrayopexpr_to_pxffilter() should
 ...
 */
	static List *append_attr_from_func_args(FuncExpr *expr, List *attrs, bool *expressionIsSupported)
	{
 ...
 */
	static List *get_attrs_from_expr(Expr *expr, bool *expressionIsSupported)
	{
		...
		*expressionIsSupported = true;
		...
		{
			/*
			 * If operand type or combination is not known, report that it's
			 * not supported
			 */
				...
				default:
					{
						 ...
						 * false-positive tuples 2) unable to join tables 3)
						 * etc
						 */
						elog(INFO,
