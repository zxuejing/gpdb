 ...
 */
static bool checkMotionWithParamWalker(Node *node, MotionWithParamContext* motionWithParamcontext)
{
	if (node == NULL)
		return false;
	...
	if (IsA(node, Motion))
	{
		...
		if (!bms_is_empty(plan->extParam))
		{
			...
			{
				elog(ERROR, "There is a param bellow Motion.");
			}
		}
	}
 ...
 */
void checkMotionWithParam(Node *node, Bitmapset *bmsNestParams, PlannerInfo *root)
{
