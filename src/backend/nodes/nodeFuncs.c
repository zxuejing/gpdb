...
#define MUTATE(newfield, oldfield, fieldtype) \
	...
	switch (nodeTag(node))
	{
		...
		case T_GroupingFunc:
		{
			...
			GroupingFunc *newnode;
			FLATCOPY(newnode, groupFunc, GroupingFunc);

			MUTATE(newnode->args, groupFunc->args, List *);
			return (Node *)newnode;
		}
		case T_GroupId:
		case T_GroupingSetId:
		...
		case T_InsertStmt:
		{
			...
			MUTATE(newnode->onConflictClause, stmt->onConflictClause, OnConflictClause *);
		...
		case T_MultiAssignRef:
		{
			MultiAssignRef *multiAssignRef = (MultiAssignRef *)node;
			MultiAssignRef *newnode;

			FLATCOPY(newnode, multiAssignRef, MultiAssignRef);
			MUTATE(newnode->source, multiAssignRef->source, Node *);
		...
		{
			LockingClause *lc = (LockingClause *)node;
			LockingClause *newnode;

			FLATCOPY(newnode, lc, LockingClause);
			MUTATE(newnode->lockedRels, lc->lockedRels, List *);
