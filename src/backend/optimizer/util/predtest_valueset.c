...
static bool TryProcessOpExprForPossibleValues(OpExpr *expr, Node *variable, Oid policy_opfamily, PossibleValueSet *resultOut);
 ...
 */
bool
TryProcessExprForPossibleValues(Node *expr, Node *variable, Oid policy_opfamily, PossibleValueSet *resultOut)
{
	if (IsA(expr, OpExpr))
	{
		return TryProcessOpExprForPossibleValues((OpExpr *) expr, variable, policy_opfamily, resultOut);
	}
...
static bool
TryProcessOpExprForPossibleValues(OpExpr *expr, Node *variable, Oid policy_opfamily, PossibleValueSet *resultOut)
{
	Node	   *leftop,
			   *rightop,
			   *varExpr;
	...
	/*
	 * if expr->opno does not belong to policy_opfamily of distributed key,
	 ...
	 */
	if (OidIsValid(policy_opfamily) && !op_in_opfamily(expr->opno, policy_opfamily))
	{
		return false;
	}
