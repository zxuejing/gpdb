...
private:
	...
	void MapLocationsFile(OID oidRel, char **rgszSegFileMap,
						  CdbComponentDatabases *pcdbCompDB);

	// segment mapping for tables with LOCATION gpfdist(s):// or custom protocol
	void MapLocationsFdist(OID oidRel, char **rgszSegFileMap,
						   CdbComponentDatabases *pcdbCompDB, Uri *pUri,
						   const ULONG ulTotalPrimaries);

	// segment mapping for tables with EXECUTE 'cmd' ON.
	void MapLocationsExecute(OID oidRel, char **rgszSegFileMap,
							 CdbComponentDatabases *pcdbCompDB,
							 const ULONG ulTotalPrimaries);
	...
	void MapLocationsExecuteOneHost(CHAR *szHostName, CHAR *szPrefixedCommand,
									char **rgszSegFileMap,
									CdbComponentDatabases *pcdbCompDB);
	...
	void MapLocationsHdfs(char **rgszSegFileMap,
						  CdbComponentDatabases *pcdbCompDB, CHAR *szFirstUri);
...
private:
	...
	WindowFrame *Pwindowframe(const CDXLWindowFrame *pdxlwf,
							  ...
							  CDXLTranslateContext *pdxltrctxOut, Plan *pplan);
	...
	RangeTblEntry *TranslateDXLTblDescrToRangeTblEntry(
		...
		const CDXLIndexDescr *index_descr_dxl, Index index,
		CDXLTranslateContextBaseTable *base_table_context);
	...
	void TranslateIndexConditions(
		...
		BOOL is_index_only_scan, BOOL is_bitmap_index_probe,
